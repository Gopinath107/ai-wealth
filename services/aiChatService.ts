...
/** Per-instrument metadata returned from the backend alongside the AI reply. */
export interface InstrumentQuote {
    instrumentKey: string;   // e.g. "NSE_EQ|INE123A01016"
    tradingSymbol: string;   // e.g. "GOLDBEES"
    name: string;            // e.g. "Nippon India ETF Gold BeES"
    exchange: string;        // "NSE" | "BSE"
    ltp: number;
    prevClose: number;
    change: number;
    changePercent: number;
    asOf: string;            // ISO-8601 with +05:30 offset
    delayed: boolean;
...
export interface SendMessageResponse {
    ...
    instrumentQuotes: InstrumentQuote[] | null;   // ← NEW
    status: string;
}
...
export const getSessionMessages = async (sessionId: number): Promise<ChatMessageData[]> => {
    ...
    try {
        const response = await fetch(
            `${BASE_URL}${ENDPOINTS.AI.SESSION_MESSAGES}/${sessionId}/messages`,
...
export const sendMessage = async (
    ...
    if (isDemoMode()) {
        ...
        const title = sessionId
            ? mockSessions.find(s => s.id === sessionId)?.title || userMessage.slice(0, 30)
            : userMessage.slice(0, 30);

        if (!sessionId) {
            mockSessions.unshift({ id: newSessionId, title, updatedAt: new Date().toISOString() });
        ...
        mockMessages[newSessionId].push({
            id: userMsgId + 1, role: 'assistant', content: aiResponse,
            createdAt: new Date().toISOString(),
        });

        return {
            sessionId: newSessionId, chatTitle: title,
            reply: aiResponse, followUps: null,
            instrumentKeys: null, instrumentQuotes: null,
            status: 'SUCCESS',
        };
    }
    ...
    try {
        ...
        return {
            ...
            instrumentQuotes: data.instrumentQuotes || null,   // ← NEW
